...
let transpose (str : event stream) (half_steps : int) : event stream =
  let rec transpose_rec (str:event stream) : event stream =
    match str() with
    | Cons(head,tail) ->
      match head with
      | Tone(d,p,v) -> 
...
let scale3 = list_to_stream (List.map eighth [(C,3);(D,3);(E,3);(F,3);(G,3);
                                            (A,3);(B,3);(C,4)]);;

let scale5 = pair scale1 scale3;;

output_midi "scale.mid" 32 scale1;;

output_midi "scales.mid" 32 scales;;

output_midi "scale5.mid" 32 scale5;;
(*>* Problem 3.4 *>*)
